...
app.post('/patientInfo/appointment/book', (req, res) => {
    ...
    //request
    var inssql = 'INSERT INTO book (pid,Username,Fname,Gender,CID,DID,DOV,Timestamp,Status,slot_id,visit_type,appointment_type) VALUES (' + patientid + ',"' + user + '","' + fname + '","' + gender + '","' + CID + '","' + DID + '","' + reqDate + '","' + Timestamp + '","' + status + '","' + slotid + '","' + visitType + '","' + appointmentType + '")';
    console.log(inssql);
...
app.post('/doctor', (req, res) => {
    ...
    //request
    var sql = "SELECT did,name,gender,experience,specialization,contact,address FROM `doctor` WHERE did =?";
    // var sql = 'SELECT * FROM patient GROUP BY date(appointmentDate)';
    pool.getConnection((err, connection) => {
        if (err) {
            console.log(err);
...
app.post('/clinic/doctordetails', (req, res) => {
    ...
    //request
    var sql = "SELECT did,name,gender,experience,specialization,contact,address FROM `doctor` WHERE did in (SELECT did FROM `doctor_availability` WHERE cid =? )";
    // var sql = 'SELECT * FROM patient GROUP BY date(appointmentDate)';
    pool.getConnection((err, connection) => {
        if (err) {
            console.log(err);
...
app.post('/getDoctorId', (req, res) => {
    ...
    var sql = 'SELECT DID FROM book where pid = ?';
    pool.getConnection((err, connection) => {
        if (err) {
            console.log(err)
        }
