...
app.post('/doctor/appointment', (req, res) => {
	...
	var currentDate=request.date;
	//request
    var sql = 'SELECT b.did,b.dov,b.pid,b.Fname,s.start_time,s.end_time FROM book b inner join doctor d on b.did=d.did  inner join slot s on b.slot_id =s.id WHERE b.did='+doctorid+' and b.DOV>="'+currentDate+'"' ;
    // var sql = 'SELECT * FROM patient GROUP BY date(appointmentDate)';
    pool.getConnection((err, connection) => {
        if (err) {
            console.log(err);
